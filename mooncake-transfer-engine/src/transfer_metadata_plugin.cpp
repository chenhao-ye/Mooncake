...
struct SocketHandShakePlugin : public HandShakePlugin {
    ...
    virtual int startDaemon(uint16_t listen_port, int sockfd) {
        ...
        // Determine IPv4 vs IPv6 (prefer IPv6, fallback to IPv4)
        bool use_ipv6 = true;
        ...
        if (sockfd >= 0) {
            ...
            listen_fd_ = socket(use_ipv6 ? AF_INET6 : AF_INET, SOCK_STREAM, 0);
            if (listen_fd_ < 0) {
...
std::vector<std::string> findLocalIpv4Addresses() {
    ...
    auto device_filter = parseDeviceList(getenv("MC_TCP_DEVICE"));
